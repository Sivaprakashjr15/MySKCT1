...
describe("MySKCT", function () {
  ...
  beforeEach(async () => {
    ...
    // Deploy contract
    MySKCT = await MySKCTFactory.deploy();
    // user1 mints an nfts 
    await MySKCT.connect(user1).mint(URI)
  })
  describe('Deployment', async () => {
    it("Should track name and symbol", async function () {
      const nftName = "MySKCT"
      const nftSymbol = "DAPP"
      expect(await MySKCT.name()).to.equal(nftName);
      expect(await MySKCT.symbol()).to.equal(nftSymbol);
    });
  })
  describe('Minting NFTs', async () => {
    it("Should track each minted NFT", async function () {
      expect(await MySKCT.tokenCount()).to.equal(1);
      expect(await MySKCT.balanceOf(user1.address)).to.equal(1);
      expect(await MySKCT.tokenURI(1)).to.equal(URI);
      // user2 mints an nft 
      await MySKCT.connect(user2).mint(URI)
      expect(await MySKCT.tokenCount()).to.equal(2);
      expect(await MySKCT.balanceOf(user2.address)).to.equal(1);
      expect(await MySKCT.tokenURI(2)).to.equal(URI);
    });
  })
  describe('Setting profiles', async () => {
    it("Should allow users to select which NFT they own to represent their profile", async function () {
      ...
      expect(await MySKCT.profiles(user1.address)).to.equal(2);
      // user 1 sets profile to first minted nft
      await MySKCT.connect(user1).setProfile(1)
      expect(await MySKCT.profiles(user1.address)).to.equal(1);
      // FAIL CASE //
      // user 2 tries to set their profile to nft number 2 owned by user 1
      await expect(
        MySKCT.connect(user2).setProfile(2)
  ...
  describe('Uploading posts', async () => {
    it("Should track posts uploaded only by users who own an NFT", async function () {
      ...
      await expect(MySKCT.connect(user1).uploadPost(postHash))
        .to.emit(MySKCT, "PostCreated")
        .withArgs(
          1,
          postHash,
          0,
      ...
      const postCount = await MySKCT.postCount()
      expect(postCount).to.equal(1);
      // Check from struct
      const post = await MySKCT.posts(postCount)
      ...
      await expect(
        MySKCT.connect(user2).uploadPost(postHash)
      ).to.be.revertedWith("Must own a MySKCT nft to post");
      // FAIL CASE #2 //
      // user 1 tried to upload a post with an empty post hash.
      await expect(
        MySKCT.connect(user1).uploadPost("")
  ...
  describe('Tipping posts', async () => {
    it("Should allow users to tip posts and track each posts tip amount", async function () {
      // user1 uploads a post
      await MySKCT.connect(user1).uploadPost(postHash)
      ...
      await expect(MySKCT.connect(user2).tipPostOwner(1, { value: tipAmount }))
        .to.emit(MySKCT, "PostTipped")
        .withArgs(
          1,
          postHash,
          tipAmount,
      ...
      const post = await MySKCT.posts(1)
      ...
      await expect(
        MySKCT.connect(user2).tipPostOwner(2)
      ).to.be.revertedWith("Invalid post id");
      // FAIL CASE #2 //
      // user 1 tries to tip their own post
      await expect(
        MySKCT.connect(user1).tipPostOwner(1)
